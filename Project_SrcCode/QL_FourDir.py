    ...
    def step(self, action):
        ...
        if [next_x,next_y] in self.obstacles:
            ...
            if self.agent_pos == self.goal_pos:
                reward = 100  # Increased goal reward
                done = True
            else:
    ...
    def render(self, screen, path=None, agent_pos=None):
        for y in range(self.grid_size):
            for x in range(self.grid_size):
                ...
                # Draw agent (red) - now properly handles tuple comparison
...
def main():
    ...
    #Track state visitation heatmap
    ...
    for episode in range(num_episodes):
        ...
        while not done and step < max_steps:
            ...
            visit_map[state[0], state[1]] += 1   # record visit count 🔥
        ...
        if (episode + 1) % 200 == 0:
            print(f"Episode {episode + 1}/{num_episodes}, Success Rate: {success_rate:.1f}%, Epsilon: {agent.epsilon:.3f}")
    ...
    for i, pos in enumerate(path):
        ...
        pygame.display.flip()
        pygame.time.delay(120)  # SPEED — smaller = faster

    # Hold display until closed
    running = True
    while running:
